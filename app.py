...
# ------------------------------------------------------------------
# หมายเหตุสำคัญเกี่ยวกับ PyQt5 / AnyQt:
# ------------------------------------------------------------------
# orangecontrib.imageanalytics (ตัวสกัดภาพ/ImageEmbedder) พึ่งพา
# แพ็กเกจ "AnyQt" อยู่ลึก ๆ ในโค้ด แม้เราจะใช้แค่ฟังก์ชันสกัดภาพผ่าน
# เซิร์ฟเวอร์ (ServerEmbedder) ก็ตาม ถ้าเครื่องที่รันแอปไม่มี PyQt5 /
# PySide2 ติดตั้งอยู่เลย จะเจอ error "PyQt4, PyQt5, PySide or PySide2
# are not available for import" ทันทีที่กดปุ่มทำนายผล
#
# วิธีแก้ที่เสถียรที่สุดคือติดตั้ง PyQt5 แบบ headless ไว้ใน
# requirements.txt (ดูไฟล์ requirements.txt) และตั้งค่าให้ Qt ทำงาน
# แบบไม่มีหน้าจอ (offscreen) เพราะเซิร์ฟเวอร์ของ Streamlit Cloud ไม่มี
# จอแสดงผล ต้องตั้งค่านี้ "ก่อน" ที่จะ import Orange ใด ๆ ทั้งสิ้น
# ------------------------------------------------------------------
    ...
    if st.session_state["model"] is None:
        ...
        try:
            ...
            st.error(
                ...
                "กรุณาตรวจสอบว่าได้ติดตั้ง Orange3, Orange3-ImageAnalytics "
                "และ PyQt5 ตามไฟล์ requirements.txt แล้ว และมีไฟล์ "
                "packages.txt (สำหรับไลบรารีระบบที่ PyQt5 ต้องใช้) อยู่ใน "
                "โปรเจกต์ด้วย จากนั้น deploy แอปใหม่อีกครั้ง"
            )
        except Exception as e:
